...
If you are either using <ins>more than 1 copies of Browser Router</ins>, there could be a very small chance for <ins>race conditions</ins> to occur because 2 copies of the same object might be trying to change the current path.
...
One solution to the above problem is to use a **Browser history instance in the container app, and child apps use Memory Router.**
However this means that you probably will need to synchronise the paths in all Router implementations:
